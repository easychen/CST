...
const basicAuthMiddleware = async function (request, response, callback, cliUsername = '', cliPassword = '') {
    ...
    if (!usePerUserAuth && 
        username === (cliUsername || config.basicAuthUser.username) && 
        password === (cliPassword || config.basicAuthUser.password)) {
        return callback();
